...
def main(args):
    ...
    trainset = datasets.load_from_disk(str(args.data_dir))
    ...
    for example, output, pred in zip(test_data, outputs, predictions):
        try:
            if float(pred) == float(example["answer"]):
    ...
    correct_set = Dataset.from_list(right_predictions)
    print(f"length of correct set : {len(correct_set)}")
    correct_set.save_to_disk(os.path.join(args.save_dir, f"train_correct"))

    with open(os.path.join(args.save_dir, f"predictions.jsonl"), "w") as fout:
        for prediction in predictions_out:
